...
def open_file(request, video_pk: int) -> tuple:
    path_str = Video.objects.get(id=video_pk).video_file.path
    path = Path(path_str)
    file = path.open('rb')
    ...
    if content_range is not None:
        ...
        range_start, range_end, *_ = map(str.strip,
                                         (content_ranges + '-').split('-'))
        range_start = max(0, int(range_start)) if range_start else 0
        range_end = min(file_size - 1,
                        int(range_end)) if range_end else file_size - 1
        content_length = (range_end - range_start) + 1
        file = ranged(file, start=range_start,
                      end=range_end + 1)
        status_code = 206
